  ...
  return (
    <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap',
      ...
      <div style={{ display: 'flex', gap: mobile ? 0 : 30,
        ...
        {[['THE ROOM', 'https://play.432bleu.com/~/concert.wam'], ['MEMBERSHIP', '/membership'], ['ARTISTS', '/artists']].map(([x, href]) =>
      ...
      <div style={{ display: 'flex', gap: mobile ? 30 : 60, flexWrap: 'wrap' }}>
        ...
        <div key={h}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {items.map((x) => Array.isArray(x) ?
            <a key={x[0]} href={x[1]} style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: 12,
              color: 'rgba(154,242,232,0.65)', letterSpacing: '0.06em', textDecoration: 'none' }}>{x[0]}</a> :
