...
<div class="about-room-main text-center py-8" transition:fly={{ x: -700, duration: 250 }}>
    ...
    <section class="container-overflow p-0">
        ...
        {#if import.meta.env.VITE_SOURCE_CODE_URL}
            <div class="mt-6 px-8 py-4 border-t border-solid border-white/20 text-left">
                <p class="text-white/70 text-sm mb-2">{$LL.menu.about.sourceCodeDescription()}</p>
                <a
                    href={import.meta.env.VITE_SOURCE_CODE_URL}
                    class="btn btn-sm btn-secondary uppercase inline-block"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    {$LL.menu.about.sourceCode()}
                </a>
            </div>
        {/if}
    </section>
